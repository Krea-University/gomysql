...
func newMySQLConfig(param MySQLConfig) *MySQLConfig {
	// fmt.Println(fmt)
	return &MySQLConfig{param.CloudSqlConnectionName, param.Host, param.Port, param.User, param.Pass, param.DbName, param.Sizeofpool, param.ErrorLog}
}
...
func (db *MySQLConnection) connect() error {
	var err error
	var dataBase *sql.DB
	if db.Config.CloudSqlConnectionName != "" {
		dataBase, err = sql.Open("mysql", fmt.Sprintf("%s:%s@unix(/cloudsql/%s)/%s", db.Config.User, db.Config.Pass, db.Config.CloudSqlConnectionName, db.Config.DbName))
	} else {
